...
// TestTransportDoesNotImplementAuthentication pins the ADR-0012 boundary: the
// transport consumes an Authenticator port and the neutral identity
// vocabulary, but never imports a concrete authentication implementation or
// policy package. Composition wires implementations in.
func TestTransportDoesNotImplementAuthentication(t *testing.T) {
	forbidden := []string{
