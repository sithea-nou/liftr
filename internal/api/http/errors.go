...
func (h *handler) mapMutationError(w http.ResponseWriter, r *http.Request, err error, id domain.ResourceID) {
	if h.service == nil {
		writeProblem(w, r, CodePersistenceUnavailable, "the control plane cannot currently reach durable state", nil)
		return
	}
	...
	if errors.Is(err, application.ErrConcurrencyConflict) {
		h.mapConcurrencyConflict(w, r, id)
		return
	}
	if errors.Is(err, lifecycle.ErrInvalidTransition) {
		h.mapStateConflict(w, r, id)
		return
	}
	mapTransportFailure(w, r, err)
}
...
func (h *handler) mapConcurrencyConflict(w http.ResponseWriter, r *http.Request, id domain.ResourceID) {
	switch {
	case r.Method == http.MethodPost:
		h.mapCreateCollision(w, r, id)
	case r.Method == http.MethodPut || r.Method == http.MethodDelete:
		...
		record, readErr := h.service.GetResource(r.Context(), id)
		if readErr == nil {
...
func (h *handler) mapStateConflict(w http.ResponseWriter, r *http.Request, id domain.ResourceID) {
...
func (h *handler) mapCreateCollision(w http.ResponseWriter, r *http.Request, id domain.ResourceID) {
	detail := "a retained Resource record already exists with this ID; tombstones are never resurrected under the same ID"
	var current *uint64
	if view, readErr := h.service.GetResourceOperation(r.Context(), id); readErr == nil && view.Resource.Resource.ID() != "" {
